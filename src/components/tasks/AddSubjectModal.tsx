...
import { cloudAddSubject, cloudUpdateSubject, getUid } from '../../lib/firestore';
...
export default function AddSubjectModal({ isOpen, onClose, editSubject }: Props) {
  ...
  useEffect(() => {
    if (editSubject && isOpen) {
      ...
      setForm({ name: '', code: '', color: getRandomSubjectColor(), creditHours: '', instructor: '', section: '' });
      setLectures([]);
    }
  }, [editSubject, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        editSubject
          ? `\u062a\u0645 \u062a\u0639\u062f\u064a\u0644 ${form.name} \u0628\u0646\u062c\u0627\u062d \u2705`
          : `\u062a\u0645\u062a \u0625\u0636\u0627\u0641\u0629 ${form.name} \u0628\u0646\u062c\u0627\u062d \u2705`,
        'success'
      );
  ...
  const addLecture = () => {
    setLectures(prev => [...prev, { dayOfWeek: 0, startTime: '08:00', endTime: '09:00', location: '' }]);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            {SUBJECT_COLORS.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-xl transition-all ${form.color === color ? 'scale-110 ring-2 ring-white/30 ring-offset-2 ring-offset-surface-card' : 'hover:scale-105'}`}
                style={{ backgroundColor: color }}
              />
            ))}
            <label
              className={`relative w-8 h-8 rounded-xl cursor-pointer transition-all flex items-center justify-center bg-surface-elevated border border-dashed border-text-muted hover:bg-surface-hover ${!SUBJECT_COLORS.includes(form.color) ? 'scale-110 ring-2 ring-white/30 ring-offset-2 ring-offset-surface-card border-none' : ''}`}
              ...
              <div key={idx} className="flex flex-col gap-2 p-2 bg-surface rounded-lg border border-surface-border">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <input
                    ...
                    className="input-field text-xs !p-1 flex-1"
                    value={lec.location || ''}
                    onChange={e => updateLecture(idx, { location: e.target.value })}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button type="submit" disabled={saving} className="btn-primary flex-1 justify-center">
            {saving ? t('saving') + '...' : (editSubject ? t('saveChanges') : t('addSubjectConfirm'))}
          </button>
        </div>
      </form>
    </Modal>
